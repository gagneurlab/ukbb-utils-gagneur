...
Provides code for extracting the regions covered by each WGS file on the RAP and then runs QC on each file.  
    - The file providing the **extracted regions** for each bcf file is here: `/s/project/deeprvat/wgs_preprocessing_rap/qc/bcf_region_files/all_region_files.parquet`
    - The **qced bcf files** (covering all protein coding genes +- 10kb) can be found here (RAP): `/ukb-gagneur/processed_data/wgs/bcf_files_qced/batch1/`.
